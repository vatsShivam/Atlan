...
class Home extends Component {
  ...
  componentDidMount() {
   ...
   const royal = this.state.data.filter(word => word.winner=="Royal Challengers Bangalore" )
    ...
    this.setState({csk:csks})
    const sun=this.state.data.filter(word => word.winner=="Sunrisers Hyderabad")
    this.setState({sunrisers:sun})
    const gujrat=this.state.data.filter(word => word.winner=="Gujarat Lions")
    this.setState({gujrat:gujrat})
    const kkr=this.state.data.filter(word => word.winner=="Kolkata Knight Riders")
    this.setState({kkr:kkr})
    const pune=this.state.data.filter(word => word.winner=="Rising Pune Supergiants")
    this.setState({pune:pune})
    const mumbai=this.state.data.filter(word => word.winner=="Mumbai Indians")
    this.setState({mumbai:mumbai})
    const punjab=this.state.data.filter(word => word.winner=="Kings XI Punjab")
    this.setState({punjab:punjab})
    const delhi=this.state.data.filter(word => word.winner=="Delhi Daredevils")
    this.setState({delhi:delhi})
    const deccan=this.state.data.filter(word => word.winner=="Deccan Chargers")
    this.setState({deccan:deccan})
    const kocchi=this.state.data.filter(word => word.winner=="Kochi Tuskers Kerala")
    this.setState({kocchi:kocchi})
    const punewarrior=this.state.data.filter(word => word.winner=="Pune Warriors")
    this.setState({punewarrior:punewarrior})
 ...
 renderTableData() {
  ...
  return this.state.data.map((btc) => {
    ...
    return (
        ...
        <h3 style={{marginTop:"10px"}}> Team1:{team1}</h3>
        <h3 style={{marginTop:"10px"}}> Team2:{team2}</h3>
        <h5 style={{marginTop:"10px"}}> Winner:{winner}</h5>
        <p class="title"> Date:{date}</p>
        <div style={{padding:"20px"}}>  
        <p>
         <Link to={"/match/" +id} shivam={season}>
  ...
  render() {
    const options = {
      title: {
        text: "Matches won by the teams in all the IPL seasons"
      },
      data: [{				
                type: "column",
                dataPoints: [
                    { label: "RCB",  y: this.state.rcb.length  },
                    { label: "CSK", y:this.state.csk.length  },
                    { label: "Sun Risers", y: this.state.sunrisers.length  },
                    { label: "Gujrat Lions",  y: this.state.gujrat.length  },
                    { label: "KKR",  y: this.state.kkr.length  },
                    { label: "Pune",  y: this.state.pune.length  },
                    { label: "Mumbai",  y: this.state.mumbai.length  },
                    { label: "Punjab",  y: this.state.punjab.length  },
                    { label: "Delhi",  y: this.state.delhi.length  },
                    { label: "Deccan",  y: this.state.deccan.length  },
                    { label: "Kochhi",  y: this.state.kocchi.length  },
                    { label: "Pune warrior",  y: this.state.punewarrior.length  },
                ]
       }]
   }
    return (
    ...
    <div className="row" >
           <div style={{backgroundColor:"#DCDCDC",marginLeft:"-20px",marginRight:"-20px"}}>
             <h3 style={{textAlign:"center",marginTop:"30px",marginBottom:"30px",}}><strong style={{fontWeight:"bold",fontSize:"30px"}}>TEAM CARDS</strong></h3>
